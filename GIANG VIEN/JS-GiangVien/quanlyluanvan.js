...
async function loadDocuments() {
  try {
    const url = `http://localhost:8080/api/documents?type=LUAN_VAN`;
...
// Xây dựng giao diện thẻ Luận văn
function renderDocuments(documents) {
  ...
  if (documents.length === 0) {
    documentListContainer.innerHTML = "<p style='text-align:center;width:100%;color:#888;'>Bạn chưa có tài liệu luận văn nào.</p>";
    return;
  }

  documents.forEach(doc => {
    const defaultCover = "https://placehold.co/300x400/1a237e/FFFFFF?text=Luận+Văn";
