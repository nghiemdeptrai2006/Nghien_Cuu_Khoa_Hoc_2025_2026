...
// Fetch single thesis by ID and open modal (for deep-linking)
async function viewThesisById(id) {
    ...
    try {
        ...
        if (res.ok && result.code === 200) {
            ...
            const author = doc.uploaderName || doc.uploadedBy || 'Sinh viên';
            const year = doc.publishYear || 'N/A';
            const extInfo = `${fileType} • ${fileSize} • Bảo vệ: ${year} • Đăng bởi: ${author}`;
            
            openViewModal(doc.title, doc.major || 'Chưa phân loại', extInfo);
        } else {
            console.error("Thesis not found or unauthorized");
        }
    } catch (error) {
...
function openEditModal(title, subject) {
    if(!title) {
        document.getElementById('uploadLuanvanForm').reset();
    } else {
...
// =========================================================
// FETCH AND RENDER THESES (LUAN_VAN)
// =========================================================
document.addEventListener('DOMContentLoaded', () => {
    loadAdminTheses().then(() => {
...
// =========================================================
let allDocuments = []; // Global storage for all documents
...
function handleCreateMajor() {
    ...
    if (!name) {
        alert("⚠️ Vui lòng nhập tên thư mục/chuyên ngành!");
        return;
    }
...
async function applyFilters() {
    ...
    await loadAdminTheses({ major, year, keyword });
}
...
async function loadAdminTheses(filters = {}) {
    const grid = document.getElementById('adminDocumentGrid');
    if (!grid) return;
    ...
    try {
        let url = 'http://localhost:8080/api/documents?type=LUAN_VAN';
...
function renderDocumentCard(doc) {
    const grid = document.getElementById('adminDocumentGrid');
    const title = doc.title || 'Luận văn không tên';
    ...
    card.innerHTML = `
        ...
        <div class="doc-info">
            ...
            <p><i class="fa-solid fa-graduation-cap"></i> Chuyên ngành: ${major}</p>
            <span>${extInfo}</span>
        </div>
...
async function saveDocument() {
    const form = document.getElementById('uploadLuanvanForm');
    ...
    try {
        ...
        if (response.ok && result.code === 200) {
            alert('✅ Cập nhật file luận văn thành công!');
            closeEditModal();
            loadAdminTheses();
        } else {
            alert('❌ Lỗi tải lên: ' + (result.message || 'Hãy kiểm tra định dạng file (chấp nhận PDF).'));
        }
    } catch (error) {
...
async function deleteDocument(docId) {
    if (!confirm('Bạn có chắc chắn muốn xóa luận văn này không?')) {
        return;
    }
    ...
    try {
        ...
        if (response.ok && result.code === 200) {
            alert('✅ Đã xóa luận văn thành công.');
            loadAdminTheses();
        } else {
            alert('❌ Xóa thất bại.');
        }
