...
async function loadDocuments() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error("Lỗi khi load luận văn:", response.status);
      documentListContainer.innerHTML = "<div class='error-msg'>Không thể tải danh sách luận văn.</div>";
    }
  } catch (error) {
  ...
  if (documents.length === 0) {
    documentListContainer.innerHTML = `
        <div style='grid-column: 1/-1; text-align:center; padding: 60px 0;'>
            <i class="fas fa-graduation-cap fa-3x" style="color: #cbd5e1; margin-bottom: 20px;"></i>
            <p style='color:#64748b; font-weight: 600;'>Chưa có hồ sơ luận văn nào được lưu trữ.</p>
        </div>`;
    return;
  }

  documents.forEach(doc => {
    const defaultCover = "https://placehold.co/300x400/6366f1/FFFFFF?text=Luận+Văn";
    ...
    card.innerHTML = `
      ...
      <div class="doc-info">
        <h3>${doc.title}</h3>
        <p><i class="fas fa-user-tag"></i> ${doc.major}</p>
        <span><i class="far fa-calendar-check"></i> Lưu trữ: ${doc.publishYear}</span>
        <div class="doc-actions">
           <button class="btn-view" onclick="openDocPreview('${doc.title}', '${doc.major}', '${doc.publishYear}', '${fileUrl}')"><i class="fas fa-file-alt"></i></button>
...
function openDocPreview(title, major, year, url) {
    ...
    document.getElementById('viewModalMeta').textContent = `Năm bảo vệ/Lưu trữ: ${year}`;
...
async function deleteDocument(id) {
    if (!confirm("⚠️ Bạn có chắc chắn muốn xóa hồ sơ luận văn này?")) return;
...
async function submitUploadForm(event) {
    ...
    if (!docFile) {
        alert("⚠️ Vui lòng chọn file luận văn đính kèm!");
        return;
    }
    ...
    if (typeof requireOtpBeforeAction === 'function') {
        requireOtpBeforeAction(async () => {
            try {
                ...
                if (response.ok) {
                    alert("✅ Đã lưu hồ sơ luận văn thành công!");
                    closeUploadModal();
                    loadDocuments();
                } else {
...
async function createFolder() {
    ...
    alert(`✅ Thư mục luận văn "${name}" đã được tạo (UI Demo)!`);
    closeFolderModal();
}
