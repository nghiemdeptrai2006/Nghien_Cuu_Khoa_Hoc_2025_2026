/* ==========================================
   GLOBAL SUPPORT BAR & CHAT LOGIC (Student)
   ========================================== */
...
function setupSupportListeners() {
    ...
    if (chatForm) {
        chatForm.addEventListener("submit", async (e) => {
            ...
            if (isDemoMode || !adminId) {
                setTimeout(() => {
                    addMessage("Cảm ơn bạn đã liên hệ! Hiện tại hệ thống đang bảo trì. Vui lòng liên hệ hotline: 0327833928 hoặc email: nguyentrongnghiem2006@gmail.com để được hỗ trợ.", "bot-msg", chatBody);
                }, 600);
                return;
            }
    ...
    document.getElementById('phoneBtn')?.addEventListener('click', () => {
        alert("📞 Hotline hỗ trợ: 0327833928");
    });
...
async function initChat() {
    ...
    if (token && token.startsWith('demo_')) {
        isDemoMode = true;
        chatBody.innerHTML = '';
        addMessage("Xin chào! Bạn đang ở chế độ Demo. Tôi là bot hỗ trợ tự động — hãy nhắn tin và tôi sẽ phản hồi!", "bot-msg", chatBody);
        return;
    }
    ...
    if (!adminId) {
        ...
        isDemoMode = true;
        chatBody.innerHTML = '';
        addMessage("Xin chào! Hệ thống hỗ trợ trực tuyến hiện không khả dụng. Tôi là bot hỗ trợ tự động — bạn có thể nhắn tin hoặc gọi hotline: 0327833928.", "bot-msg", chatBody);
    } else {
        loadHistory();
        startPolling();
    }
